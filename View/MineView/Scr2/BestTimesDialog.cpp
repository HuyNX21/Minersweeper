...
void BestTimesDialog::showBestTimes(const std::vector<BestTimeEntry> times)
{
    ...
    for (int i = 0; i < times.size(); ++i)
    {
        ...
        auto* mineItem = new QTableWidgetItem(QString::fromStdString(t.minefieldText()));
        auto* playItem = new QTableWidgetItem(QString::fromStdString(t.playerName));
...
void BestTimesDialog::enterWinMode(const BestTimeEntry entry)
{
    ...
    auto* mineItem = new QTableWidgetItem(QString::fromStdString(entry.minefieldText()));
...
void BestTimesDialog::onDoneClicked()
{
    ...
    const std::string playerName = editor->text().trimmed().toStdString();
    if (playerName.empty())
        return;
